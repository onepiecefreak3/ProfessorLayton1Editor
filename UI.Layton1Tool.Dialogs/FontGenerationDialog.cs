...
internal partial class FontGenerationDialog
{
    ...
    private readonly NftrData _fontData;
    ...
    public FontGenerationDialog(NftrData fontData, FontGenerationType type, string? selectedCharacters, IEventBroker eventBroker,
        IComponentFactory components, ILocalizationProvider localizations, ISettingsProvider settings)
    {
        InitializeComponent(type, selectedCharacters, components, localizations, settings);
        ...
        _profile = new FontProfile
        {
            ...
            Characters = localizations.FontGenerateDefaultCharacters,
            Paddings = new Dictionary<char, (int, int)>()
        };

        RaiseUpdatePaddedGlyphZoom(20f);
    }

    private void RaiseUpdatePaddedGlyphZoom(float zoom)
    {
        _eventBroker.Raise(new UpdatePaddedGlyphZoomMessage(_glyphBox, zoom));
    }

    private void RaiseSelectedPaddedGlyphChanged(PaddedGlyph? paddedGlyph)
    {
        _eventBroker.Raise(new SelectedPaddedGlyphChangedMessage(_glyphBox, paddedGlyph));
    }

    private void _replaceCharactersCheck_CheckChanged(object? sender, EventArgs e)
    {
        _settings.ReplaceFontCharacters = _replaceCharactersCheck.Checked;
    }
    ...
    private async void _loadBtn_Clicked(object? sender, EventArgs e)
    {
        ...
        {
            Title = _localizations.DialogFontGenerateLoadCaption,
            Filters = new List<FileFilter>
            {
                new(_localizations.DialogFontGenerateProfile, "bfgp")
            }
        };
    ...
    private async void _saveBtn_Clicked(object? sender, EventArgs e)
    {
        ...
        {
            Title = _localizations.DialogFontGenerateSaveCaption,
            Filters = new List<FileFilter>
            {
                new(_localizations.DialogFontGenerateProfile, "bfgp")
            }
        ...
        foreach (char character in _characterEditor.GetText().Distinct().Order())
        {
            ...
            if (char.IsWhiteSpace(character))
            {
                characterInfo = new CharacterInfo
                {
                    ...
                    BoundingBox = new SixLabors.ImageSharp.Size(_profile.SpaceWidth, _profile.GlyphHeight)
                };
            }
            else
            {
                ...
                characterInfo = new CharacterInfo
                {
                    ...
                    GlyphPosition = paddedGlyph.GlyphPosition,
                    BoundingBox = paddedGlyph.BoundingBox
                };
            }
    ...
    private void EditFont()
    {
        ...
        foreach (char character in _characterEditor.GetText().Distinct().Order())
        {
            ...
            CharacterInfo? characterInfo = _fontData.Characters.FirstOrDefault(c => c.CodePoint == character);
    ...
    private void UpdateGlyph(char character)
    {
        ...
        RaiseSelectedPaddedGlyphChanged(paddedGlyph);
    }
