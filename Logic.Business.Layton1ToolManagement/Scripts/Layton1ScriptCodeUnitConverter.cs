...
internal class Layton1ScriptCodeUnitConverter(ILayton1ScriptInstructionDescriptionProvider layton1ScriptInstructionProvider) : ILayton1ScriptCodeUnitConverter
{
    ...
    public GdsScriptFile CreateScriptFile(CodeUnitSyntax tree, string gameCode)
    {
        ...
        AddFunctions(instructions, tree.MethodDeclarations, gameCode);
    ...
    private void AddFunctions(List<GdsScriptInstruction> instructions, IReadOnlyList<MethodDeclarationSyntax> methods, string gameCode)
    {
        if (methods.Count <= 0)
            return;

        AddStatements(instructions, methods[0].Body.Expressions, gameCode);
    }
            ...
            switch (statement)
            {
                ...
                case MethodInvocationStatementSyntax methodInvocation:
                    if (IsCommand(methodInvocation))
                        ...
                        AddMethodInvocationStatement(instructions, methodInvocation, jump, gameCode);
                    break;
    ...
    private void AddMethodInvocationStatement(List<GdsScriptInstruction> instructions, MethodInvocationStatementSyntax methodInvocation, GdsScriptJump? jump, string gameCode)
    {
        int invocationType = GetInvocationType(methodInvocation.Identifier, gameCode);
