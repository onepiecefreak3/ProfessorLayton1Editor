...
class Frame1Reader : IFrame1Reader
{
    public Frame1Container Read(Stream input)
    {
        ...
        var paletteData = ReadPaletteData(reader);
    ...
    private Frame1ImageEntry[] ReadImageEntries(BinaryReaderX reader, AnimationReaderContext context)
    {
        var result = new Frame1ImageEntry[context.FrameCount];
    ...
    private Frame1ImageEntry ReadImageEntry(BinaryReaderX reader)
    {
        var width = reader.ReadInt16();
        var height = reader.ReadInt16();
        var partCount = reader.ReadInt32();

        var entry = new Frame1ImageEntry
        {
            width = width,
            height = height,
            partCount = partCount
    ...
    private Frame1ImageSubEntry[] ReadImageSubEntries(BinaryReaderX reader, AnimationReaderContext context)
    {
        var result = new Frame1ImageSubEntry[context.PartCount];
    ...
    private Frame1ImageSubEntry ReadImageSubEntry(BinaryReaderX reader)
    {
        return new Frame1ImageSubEntry
        {
            x = reader.ReadInt16(),
            y = reader.ReadInt16(),
    ...
    private int DetectImageFormat(BinaryReaderX reader, Frame1ImageSubEntry subEntry, AnimationReaderContext context)
    {
    ...
    private bool ApplyImageFormat(BinaryReaderX reader, Frame1ImageSubEntry subEntry, AnimationReaderContext context, int imageFormat)
    {
        ...
        else
        {
            ...
            reader.BaseStream.Position += ((colorCount * 2 + 3) & ~3) + 0x1E;
            var animationCount = reader.ReadInt32();
    ...
    private int GetDataSize(Frame1ImageSubEntry subEntry, int imageFormat)
    {
    ...
    private byte[] ReadPaletteData(BinaryReaderX reader)
    {
        var colorCount = reader.ReadInt32();
